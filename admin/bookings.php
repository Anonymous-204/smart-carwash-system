    ...
    if ($new === 'COMPLETED' && $old !== 'COMPLETED') {
        ...
        if ($res_order && $row_order = $res_order->fetch_assoc()) {
            ...
            if ($points_earned > 0 && $customer_id > 0) {
                // 1. Cập nhật cộng điểm vào bảng users (sử dụng đúng column name là 'point')
                $conn->query("UPDATE users SET point = COALESCE(point, 0) + $points_earned WHERE id = $customer_id");

                // 2. Lấy số điểm mới nhất để xét hạng
                $res_pts = $conn->query("SELECT point FROM users WHERE id = $customer_id");
                if ($res_pts && $row_pts = $res_pts->fetch_assoc()) {
                    $current_points = (int)$row_pts['point'];
                    
                    // 3. Call the helper function to update rank automatically
                    update_customer_rank($conn, $customer_id);
                }
            }
        }
    }
