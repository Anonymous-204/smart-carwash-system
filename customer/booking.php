...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_vehicle'])) {
    ...
    if ($brand && $vehicleType && $licensePlate) {
        ...
        $stmt->execute();
        $stmt->close();
        // PRG: redirect để tránh resubmit khi reload
        header('Location: ' . $_SERVER['PHP_SELF'] . '?msg=vehicle_added');
        exit;
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_order'])) {
    ...
    if ($order && in_array($order['status'], ['PENDING', 'CONFIRMED'])) {
        ...
        $stmt->close();
        // PRG
        header('Location: ' . $_SERVER['PHP_SELF'] . '?msg=cancelled');
        exit;
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['book_now'])) {
    ...
    if (empty($errors)) {
        ...
        if (!$svc) {
            ...
            // PRG: redirect sau khi đặt lịch thành công
            header('Location: ' . $_SERVER['PHP_SELF'] . '?msg=booked');
            exit;
        }
    }
}
...
// ---------------------------------------------------------------
// Đọc message từ query string (sau redirect)
// ---------------------------------------------------------------
if (isset($_GET['msg'])) {
    switch ($_GET['msg']) {
        case 'booked':       $message = 'Đặt lịch thành công! Chúng tôi sẽ xác nhận sớm.'; break;
        case 'cancelled':    $message = 'Đã hủy lịch thành công.'; break;
        case 'vehicle_added': $message = 'Đã thêm xe thành công.'; break;
    }
}
                            ...
                            ?>
                                <option value="<?= $s['id'] ?>"
                                    ...
                                    <?php if ($pricing['discount_percent'] > 0): ?>
                                        <?= money($pricing['original']) ?> → <?= money($pricing['final']) ?> (-<?= $pricing['discount_percent'] ?>%)
                                    <?php else: ?>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const orderIds = Array.from(orderBlocks).map(b => b.getAttribute('data-order-block'));
        ...
        setInterval(function () {
            ...
            .then(data => {
                ...
                data.orders.forEach(order => {
                    ...
                    if (cancelContainer && order.status !== 'PENDING' && order.status !== 'CONFIRMED') {
    ...
    // Price breakdown on service select
    const serviceSelect = document.getElementById('serviceSelect');
    if (serviceSelect) {
        serviceSelect.addEventListener('change', function () {
            const opt = this.options[this.selectedIndex];
            const originalPrice  = opt.dataset.originalPrice;
            const finalPrice     = opt.dataset.finalPrice;
            const discountAmount = opt.dataset.discountAmount;
            const discountPct    = opt.dataset.discountPercent;

            const breakdown = document.getElementById('priceBreakdown');
            if (!originalPrice) { breakdown.style.display = 'none'; return; }

            const fmt = v => new Intl.NumberFormat('vi-VN').format(v) + ' đ';
            breakdown.style.display = 'block';
            document.getElementById('originalPrice').textContent = fmt(originalPrice);
            document.getElementById('finalPrice').textContent    = fmt(finalPrice);

            const discountRow = document.getElementById('discountRow');
            if (discountPct > 0) {
                discountRow.style.display = 'block';
                document.getElementById('discountPercent').textContent = discountPct;
